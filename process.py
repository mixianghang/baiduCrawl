...
def cut_sentence(filepath):
    ...
    while enable:
        ...
        start = 0
        i = 0
        # punt_list = ',.!?:;~，。！？：；～ \n'.decode('utf-8') # 最后包含了一个空格,前面加u.就不用decode
        punt_list = u'.!?;:~。！？；～\n' # 不从逗号/空格分开句子
        for word in line:
...
def uniSegSents(sent_seg_list,productA,productB):
    desA = productA[0]
    desB = productB[0]
    compare_word = [u'对比',u'比较',u'v',u'vs',u'V',u'VS',u'相比',u'相对',u'超过',u'强于',u'高于',u'好于',u'优于',u'大于']
    for s in sent_seg_list:
        for p in productA:
...
if __name__=="__main__":
    ...
    txtpath = raw_input(u'enter the absolute or relative abstract file path for process:')
    sents = cut_sentence(txtpath)
    sent_seg_list = segSent(sents)
    
    productA = [u'苹果',u'iPhone',u'iphone',u'iphone5s',u'iphone5',u'iphone4',u'iphone4s',u'iPhone5',u'iPhone5s',u'iphone6',u'iPhone6']
